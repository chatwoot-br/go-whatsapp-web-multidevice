...
func (r *DeviceRepository) MergeLIDChat(deviceID, lidJID, phoneJID string) error {
	target := deviceID
	if target == "" {
		target = r.deviceID
	}
...
func (r *DeviceRepository) GetLIDChats(deviceID string) ([]*domainChatStorage.Chat, error) {
	target := deviceID
	if target == "" {
		target = r.deviceID
	}
