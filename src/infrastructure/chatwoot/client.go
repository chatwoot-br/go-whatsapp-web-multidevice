...
func (c *Client) FindContactByIdentifier(identifier string, isGroup bool) (*Contact, error) {
	...
	for _, contact := range result.Payload {
		if isIdentifierBased {
			...
			// Back-compat: contacts created before the waha→gowa custom-attribute
			// rebrand stored the JID under waha_whatsapp_jid. Existing contacts
			// match by Identifier/phone and never get rewritten, so keep resolving
			// the legacy key indefinitely (fork-only divergence from upstream).
