...
func (h *ChatwootHandler) resolveChatwootWebhookRoute(payload chatwoot.WebhookPayload) chatwootWebhookRoute {
	...
	// Back-compat: contacts created before the waha→gowa custom-attribute rebrand
	// stored the WhatsApp JID under waha_whatsapp_jid; resolve it so agent replies
	// to pre-existing contacts still route (they never get rewritten to gowa).
	if route.Destination == "" {
		route.Destination = chatwootContactAttrString(contact.CustomAttributes, "waha_whatsapp_jid")
	}
