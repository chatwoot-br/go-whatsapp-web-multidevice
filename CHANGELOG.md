...
- **A failed logout is retryable** — keep-slot logout cleared the in-memory JID even when the whatsmeow store cleanup failed, so the retry arrived with no JID, deleted nothing, and reported success — leaving the orphan row that `LoadExistingDevices` resurrects on restart. The retry now recovers the identity from the persisted device record, preferring the live `jid` (the reset never landed) and falling back to `last_jid` (the reset landed but the store delete failed) — the column that still holds it depends on how far the failed attempt got, and reading only `last_jid` would have let the retry clear the row's `jid` without ever recording it, stranding that account's chat data with no pointer left to name it.
...
- **Logout never leaves the device row naming no account** — `resetDeviceKeepSlot` cleared the persisted `jid` *before* recording `last_jid`, so a failure in between (superseded-data purge, `SetDeviceLastJID`) left `jid=''` with a stale `last_jid`, and the retry then recovered the *previous* account — permanently losing the pointer to the current one's retained chat data. The live `jid` is now cleared last, so every intermediate failure leaves the account recoverable.
- **`DELETE /devices/{jid}` works after a logout** — a logged-out slot has no live JID, so resolving the caller's JID could not find the uuid-keyed slot; the purge deleted the chat data but removed nothing else and reported success while the real slot stayed listed and persisted. The JID is now also resolved via `last_jid`.

### Fixed (Codex review round 8)
- **`history_sync_complete` reaches device-only deployments** — the fork's own event (the one `chatwoot-app` waits on to know a fresh pairing's backfill has settled) armed its debounce timer only when a **global** `--webhook` was configured, so an instance whose consumer is a *per-device* webhook never scheduled it and the documented event never arrived. Both history-sync paths now use the device-aware consumer test, and the event name is a shared constant so the scheduler and the dispatcher cannot disagree about it.
- **A webhook on a JID-keyed slot is found** — an auto-created slot is keyed *by* the JID (`device_id = "<jid>"`) and `handleConnectionEvents` deliberately skips persisting its `jid` column (it would recreate deleted duplicates), so such a row can sit at `device_id=<jid>, jid=''`. The config lookup searched only the `jid` column, so a webhook configured on that slot before first pairing was never found and its events fell through to the global/no-op path. It now falls back to the slot-id lookup.
- **Passkey pairings with `skip_handoff_ux` actually complete** — that flag means the *code* need not be shown and verified, not that the confirmation call can be skipped; whatsmeow still requires it, so those pairings stayed pending forever with only a toast shown. The UI now posts the confirm request (without the dialog).
- **The passkey device header is URL-encoded** — `DeviceMiddleware` `url.QueryUnescape`s `X-Device-Id` and the selected-device path encodes it, so the raw override could resolve a different or missing device.

### Fixed (Codex review round 7)
- **The pre-payload gate now asks about the exact event, and about Chatwoot's real filters** — two remaining holes in it: (a) "Chatwoot is enabled" was treated as a consumer outright, but Chatwoot mirrors only the events `shouldForwardEventToChatwoot` covers and only within the global whitelist, so a deployment whose `WHATSAPP_WEBHOOK_EVENTS` excludes messages still downloaded media for messages nothing could receive; (b) the gate asked whether *any* message-family event was accepted, so a whitelist admitting only `message.reaction` still pulled ordinary media messages into payload construction. `ClassifyMessageEvent` now derives the exact event name by struct inspection (no media work), and the gate mirrors `forwardPayloadToConfiguredWebhooks`' own `webhookAllowed || chatwootAllowed` decision. `buildEventPayload` calls the same classifier, so the gate and delivery cannot drift apart (pinned by a test).
- **The passkey UI confirms the device the event names** — the confirm POST used the globally-selected `X-Device-Id`, so with several devices open (or a pairing started from another tab/the API) the backend checked the wrong slot, found no pending confirmation, and left the real pairing stuck. It now sends the broadcast's `device_id`.

### Fixed (Codex review round 6)
- **`DELETE /devices/{jid}` can no longer destroy a re-paired live account (P1)** — `last_jid` deliberately survives a re-pairing (it is the only pointer to the chat data the logout retained), so a slot logged out of A and since re-paired to B holds `jid=B` *and* `last_jid=A`. The `last_jid` lookup matched it regardless, so a DELETE naming **A** was handed the live **B** slot and purged it — logging out and destroying an account the caller never named. A `last_jid` match now counts only while the slot is genuinely logged out (empty live jid, persisted and in memory).
- **The pre-payload gate honors the device's event filter** — a device whose `webhook_events` excludes the message family (say `call.offer` only) still has a URL, so the gate admitted it and downloaded media for a payload the whitelist then dropped. The gate now asks whether any message-family event (`message`, `message.reaction`, `message.revoked`, `message.edited`) can actually reach a destination for that device, mirroring the destination selection in `forwardPayloadToConfiguredWebhooks`.
- **MCP send tools normalize bare recipients** — `ui/mcp/send.go` never called `utils.SanitizePhone` (unlike the REST handlers and the `query`/`group` MCP families), so a bare group id parsed as an `@s.whatsapp.net` user JID and the send was addressed to a non-existent contact instead of the `@g.us` group. All 10 send handlers now sanitize, not just the four tools this sync adds.

### Fixed (Codex review round 5)
- **A logged-out slot can no longer be rebound to a different WhatsApp account** — `LoadExistingDevices` treated *any* empty-JID slot as a never-paired placeholder and attached the first unmatched whatsmeow store row to it. A keep-slot logout leaves exactly that shape, so on an install that also had a legacy/store-only session for another account, the logged-out slot was silently adopted by that unrelated JID — and every later reconnect/logout/delete on it acted on the **wrong account**. `last_jid` is now carried in memory and marks such a slot as owned; only a slot with neither a live jid nor a `last_jid` may adopt a row (an unmatched row still surfaces as its own instance).
- **Deleting a device drops its cached webhook config** — the per-device caches are keyed by JID and cache nil results too, so a purged device's webhook URL kept receiving events for the whole TTL; re-pairing the same account shortly after a delete forwarded to the deleted device's endpoint. `RemoveDevice` (and keep-slot logout) now invalidate the caches.
- **Submitting a passkey assertion no longer discards the confirmation code** — `PairPasskeyConfirmation` can arrive *before* `SendPasskeyResponse` returns, and clearing all passkey state afterwards wiped the code the event had just stored, leaving no pending status and nothing to pass to `/app/passkey/confirm`. The consumed challenge is now retired on its own (`ClearPasskeyChallenge`).

### Fixed (per-device webhook semantics — upstream-forwardable)

Upstream's per-device webhook config (#671) merges field-by-field into the global config, so a device's *zero values* silently fall back to global settings that do not own its URL. A non-nil device config is now **authoritative for the URL it owns**; the global path (no device config) is byte-identical, so the fork's Chatwoot webhook contract — global URL, global secret, global whitelist — is unchanged.

- **`webhook_insecure_skip_verify=false` restores TLS verification** for a device webhook even when the global `WHATSAPP_WEBHOOK_INSECURE_SKIP_VERIFY` is true (previously the flag could only ever be turned *on*, so one self-signed global webhook disabled certificate checks for every device webhook).
- **An empty device `webhook_secret` signs with the device secret, not the global one** — a consumer reading its (empty) secret from `GET /devices/{id}/webhook` could not validate the HMAC unless it was also handed the unrelated global key.
- **An empty device `webhook_events` means all events** (the semantics openapi.yaml documents, and the same "empty = no filter" rule the global list follows) instead of inheriting the global whitelist, which silently dropped receipts/calls/groups from a device webhook that asked for no filter.

### Known upstream issues (verified, reported — not fixed here; per-device-webhook users only)
- `PATCH /devices/:id/webhook` has PUT semantics (plain fields — omitting `webhook_secret`/`webhook_events` wipes them); a URL-less webhook config is accepted and echoed but never applied; MCP mode never drains `websocket.Broadcast`, so passkey/pair events stall whatsmeow's handler queue ~5min each (pre-existing, more send sites now); with **no** webhook/Chatwoot consumers configured, every media message is still downloaded to `statics/media` for a payload that is then discarded; (Two of these are fixed fork-side and remain open upstream: the no-consumer media download — see the device-scoped consumer gate — and the `GetDeviceRecordByJID` duplicate-row ordering.)
